 ...
 *   0    => 0
 */
function getCircleCircumference(P) {
  return P * (2 * Math.PI);
}
...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const firstStep = x1 * x2 + y1 * y2;
  const secStep = Math.sqrt(x1 ** 2 + y1 ** 2) * Math.sqrt(x2 ** 2 + y2 ** 2);
  const arrow = Math.acos(firstStep / secStep);
...
function getFibonacciNumber(index) {
  ...
  for (let i = 3; i <= index; i += 1) {
    const element = arrFib[i - 1] + arrFib[i - 2];
    arrFib.push(element);
  }
...
function getSumOfDigits(num) {
  const string = num.toLocaleString().split('');
  let sum = 0;
  let tick = 0;
  while (tick < string.length) {
    sum += +string[tick];
    tick += 1;
  }
  return sum;
